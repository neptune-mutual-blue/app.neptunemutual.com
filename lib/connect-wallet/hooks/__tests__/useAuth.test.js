...
import { UserRejectedRequestErrorInjected } from "@/lib/connect-wallet/injected/errors";
...
describe("Connect Wallet Hooks", () => {
  ...
  test("Connect to Unknown", async () => {
    ...
    mockFn.useWeb3React(() => ({ ...testData.account, connector }));
    const { result } = renderHook(() =>
      useAuth(testData.network.networkId, notify)
    );
  ...
  test("Connect to Injected and login", async () => {
    ...
    result.current.login(ConnectorNames.Injected);
  ...
  test("Connect to BSC with Error", async () => {
    ...
    result.current.login(ConnectorNames.BSC);
