...
function getMonthsBetweenDates (locale, startDate, endDate) {
  ...
  while (currentDate.getTime() <= endDate) {
    months.push(formatInsightsMonthId(locale, currentDate))
...
function formatInsightsMonthId (locale, date, overrides = {}) {
  return new Date(date).toLocaleString(locale, { month: 'short', year: '2-digit', ...overrides })
}

export { formatInsightsMonthId, getMonthNames, getMonthsBetweenDates }
