...
const formatValue = (options) => {
    ...
    const isNegative = new RegExp(`^\\d?-${prefix ? `${escapeRegExp_1.escapeRegExp(prefix)}?` : ''}\\d`).test(_value);
    ...
    const intlSuffix = getSuffix_1.getSuffix(formatted, Object.assign({}, options));
    ...
    if (decimalScale === undefined && decimals && decimalSeparator) {
        if (formatted.includes(decimalSeparator)) {
            formatted = formatted.replace(RegExp(`(\\d+)(${escapeRegExp_1.escapeRegExp(decimalSeparator)})(\\d+)`, 'g'), `$1$2${decimals}`);
        }
        else {
...
const replaceDecimalSeparator = (value, decimalSeparator, isNegative) => {
    ...
    if (decimalSeparator && decimalSeparator !== '.') {
        newValue = newValue.replace(RegExp(escapeRegExp_1.escapeRegExp(decimalSeparator), 'g'), '.');
