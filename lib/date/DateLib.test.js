...
describe("DateLib methods", () => {
  ...
  test("toDateTimeLocal", () => {
    ...
    ).toEqual(`2022-01-01T${addPadding(hours)}:00`);

    expect(DateLib.toDateTimeLocal(DateLib.toUnix(date))).toEqual(
      `2022-01-01T${addPadding(hours)}:00`
    );

    expect(DateLib.toDateTimeLocal()).toEqual(
      `2022-${addPadding(month)}-${addPadding(day)}T${addPadding(
        hours
      )}:${addPadding(minutes)}`
    );
  });
...
function addPadding(num, length = 2) {
