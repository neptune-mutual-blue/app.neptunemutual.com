...
class DateLib {
  ...
  static durationBetween(startDate, endDate) {
    ...
    if (diffDays >= 1) {
      return { duration: diffDays, durationAgo: t`in ${diffDays} days` };
    }

    if (diffHrs >= 1) {
      return { duration: diffHrs, durationAgo: t`in ${diffHrs} hours` };
    }

    if (diffMins > 0) {
      return { duration: diffMins, durationAgo: t`in ${diffMins} minutes` };
    }

    if (diffDays <= 1) {
      return {
        duration: diffDays,
        durationAgo: t`${Math.abs(diffDays)} days ago`,
      };
    }
    ...
    if (diffHrs <= 1) {
      return {
        duration: diffHrs,
        durationAgo: t`${Math.abs(diffHrs)} hours ago`,
      };
    }

    return { duration: diffMins, durationAgo: t`${Math.abs(diffMins)} ago` };
  }
}
