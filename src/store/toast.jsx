...
export const useToastsStore = create((set, get) => ({
  ...
  pushError: ({ message, title = "Error", lifetime }) => {
    get().push({ message, type: "Error", title, lifetime });
  },
  pushWarning: ({ message, title = "Warning", lifetime }) => {
    get().push({ message, type: "Warning", title, lifetime });
  },
  pushSuccess: ({ message, title = "Success", lifetime }) => {
    get().push({ message, type: "Success", title, lifetime });
  },
  pushInfo: ({ message, title = "Info", lifetime }) => {
    get().push({ message, type: "Info", title, lifetime });
  },
  pushLoading: ({ message, title = "Loading", lifetime }) => {
    get().push({ message, type: "Loading", title, lifetime });
  },
  txPush: async ({ tx, titles, options, networkId }) => {
    const { pushLoading, pushSuccess, pushError } = get();
...
export const toast = useToastsStore.getState().push;
