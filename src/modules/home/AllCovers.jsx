...
import { useQueryCovers } from "@/src/hooks/useQueryCovers";
...
export const AllCovers = () => {
  const { data: availableCovers, loading } = useQueryCovers();
    ...
    <Container className="py-16" data-testid="available-covers-container">
      ...
      <Grid className="gap-4 mt-14 lg:mb-24 mb-14">
        ...
        {sortedCovers.map((c, idx) => {
          ...
          const isDiversified = c?.products?.length;
          ...
          return (
            <Link
              href={
                isDiversified
                  ? `/${cover_id}`
                  : `/${getBasePath(basePathArray)}/${cover_id}/options`
              }
              key={c.id}
            >
              <a
