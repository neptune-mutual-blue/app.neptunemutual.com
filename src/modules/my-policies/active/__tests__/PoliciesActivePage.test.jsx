...
const getMockActivePolicies = async () => {
  try {
    const res = await fetch('https://api.thegraph.com/subgraphs/name/neptune-mutual/subgraph-mumbai', {
      method: 'POST',
      body: JSON.stringify({
        query: `
        {
...
const mockActivePolicies = [
  {
    cover: { id: '0x6262382d65786368616e67650000000000000000000000000000000000000000' },
    cxToken: {
...
describe("PoliciesActivePage", () => {
  describe("should render PoliciesActivePage", () => {
    ...
    test("it has Transaction List link", () => {
      ...
      const TransactionListLink = getByRole("link", { name: /Transaction List/i });
    ...
    test('should placeholder when no active policies', () => {
      ...
      const text = 'A cover policy enables you to claim and receive payout when an incident occurs. To purchase a policy, select a cover from the home screen.';
